...
def test_single_user(test_app, test_database, add_user):
    user = add_user("jeffrey", "jeffrey@testdriven.io", "password1")
    client = test_app.test_client()
...
def test_all_users(test_app, test_database, add_user):
    ...
    add_user("michael", "michael@mherman.org", "password1")
    add_user("fletcher", "fletcher@notreal.com", "password1")
    client = test_app.test_client()
...
def test_remove_user(test_app, test_database, add_user):
    test_database.session.query(User).delete()
    user = add_user("user-to-be-removed", "remove-me@testdriven.io", "password1")
    client = test_app.test_client()
...
def test_update_user(test_app, test_database, add_user):
    user = add_user("user-to-be-updated", "update-me@testdriven.io", "password1")
    client = test_app.test_client()
    resp_one = client.put(
        f"/users/{user.id}",
        data=json.dumps(
            {"username": "me", "email": "me@testdriven.io", "password": "password1"}
        ),
        content_type="application/json",
    )
    data = json.loads(resp_one.data.decode())
